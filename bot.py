    ...
    async def bannew_command(interaction: discord.Interaction):
        ...
        async with aiohttp.ClientSession() as session:
            ...
            if not source_universe_ids and not source_errors:
                await interaction.followup.send(
                    "checked all configured universes and none of them have active bans right now.",
                    ephemeral=True,
                )
                return
            ...
            if not empty_universe_ids:
                await interaction.followup.send(
                    "checked all universes and there are no empty targets. every universe already has at least one active ban.",
                    ephemeral=True,
                )
                return
